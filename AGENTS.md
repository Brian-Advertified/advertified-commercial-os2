...
The owner grants standing deployment authority for requested Advertified implementation work on 2026-09-08. Build, test and deploy verified changes to the existing local Docker development stack without repetitive deployment approval. Deployment to an explicitly selected production environment follows its retained readiness gates; this standing authority does not choose a new cloud target, waive failed checks, authorise new paid resources or expose production data. Never commit, push, merge, mutate unrelated cloud resources or contact an external party without the owner's explicit authority for those actions. Preserve existing data and do not rerun development seeds during application refreshes.

Use only the storage needed for the authorised work. Check actual host free space and Docker disk use before and after builds; avoid parallel heavy builds, persistent validation images, unnecessary raster assets, screenshots, traces and duplicate dependency/output caches. Prefer the pinned memory-backed API verifier and reuse installed dependencies. Keep build cache bounded and retain only necessary runtime/toolchain and rollback artefacts. Pause disk-heavy work before exhausting the host reserve; report host free bytes, not only Docker's logical reclamation. Never delete data volumes, inventory, user records or recovery backups to meet a storage target. Docker VHD compaction must run offline against the verified Docker data disk with administrator access, followed by actual host-space and data/health verification.

The owner authorised live AI/provider calls for the current planner-intelligence work on 2026-09-08 and explicitly increased the aggregate ceiling to US$10 total on 2026-09-13 (ADVERTIFIED.md section 49.3.15). This is not per agent, request, day or month; all earlier reservations and uncertain calls remain counted. Before any paid AI invocation, enforce a shared remaining-budget check that includes in-flight reservations and conservative maximum request cost; if cost cannot be bounded, do not invoke. This does not authorise research subscriptions, commercial commitments or unrestricted background processing. Provider switches remain disabled by default and are enabled deliberately only for authorised work. Tests must use deterministic local fixtures or fakes. Never broaden a permission, budget, audience, tenant scope, or commercial action to get past a blocker.
