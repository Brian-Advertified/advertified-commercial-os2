    ...
    ITenantAuthorizer authorizer) : IProposalReader
{
    ...
    {
        await RequireAsync(actorId, tenantId, MasterDataReferences.Permissions.ProposalView, cancellationToken);
        ...
        var proposal = await store.FindProposalAsync(tenantId, proposalVersionId, cancellationToken)
    ...
    public async Task<IReadOnlyList<ApprovedPlanChoiceView>> ListApprovedPlansAsync(
        ...
        CancellationToken cancellationToken)
    {
        await RequireAsync(actorId, tenantId, MasterDataReferences.Permissions.ProposalGenerate, cancellationToken);
        await using var transaction = await store.BeginSessionAsync(actorId, tenantId, cancellationToken);
        var brief = await store.FindApprovedBriefAsync(tenantId, briefId, cancellationToken)
        ...
        var views = new List<ApprovedPlanChoiceView>(rows.Count);
        foreach (var row in rows)
        {
            var plan = await planningStore.BuildPlanViewAsync(tenantId, row, cancellationToken);
            views.Add(new ApprovedPlanChoiceView(
    ...
    public async Task<IReadOnlyList<ProposalRecipientView>> ListRecipientsAsync(
        ...
        CancellationToken cancellationToken)
    {
        await RequireAsync(actorId, tenantId, MasterDataReferences.Permissions.ProposalShare, cancellationToken);
        await using var transaction = await store.BeginSessionAsync(actorId, tenantId, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var canPrepare = await CanPrepareAsync(actorId, tenantId, cancellationToken);
        await using var transaction = await store.BeginSessionAsync(actorId, tenantId, cancellationToken);
        var document = await store.FindDocumentByIdAsync(tenantId, documentId, cancellationToken)
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!decision.IsAllowed) throw new UnauthorizedAccessException("Proposal access denied.");
    }
}
