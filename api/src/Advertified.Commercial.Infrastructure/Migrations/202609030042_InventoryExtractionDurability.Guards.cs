...
{
    private static void AddAttemptGuards(MigrationBuilder migrationBuilder)
    {
        migrationBuilder.Sql(
            """
            CREATE FUNCTION commercial.enforce_inventory_extraction_attempt_transition()
            RETURNS trigger
            LANGUAGE plpgsql
            ...
            END;
            $attempt_transition$;
            CREATE TRIGGER enforce_inventory_extraction_attempt_transition
                BEFORE UPDATE ON commercial.inventory_extraction_attempts
                FOR EACH ROW EXECUTE FUNCTION
                    commercial.enforce_inventory_extraction_attempt_transition();
            """);
    }
}
