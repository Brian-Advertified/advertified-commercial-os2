...
{
    private static void RestoreBookingOnlyCampaignBoundary(MigrationBuilder migrationBuilder) =>
        migrationBuilder.Sql(
            """
            DO $creative_rollback_guard$
            BEGIN
                IF EXISTS (
                    SELECT 1 FROM commercial.campaigns
                    WHERE status_code IN ('CREATIVE_PENDING', 'READY')) THEN
                    RAISE EXCEPTION
                        'creative readiness migration cannot roll back while creative campaigns exist';
                END IF;
            END;
            $creative_rollback_guard$;
