...
[Migration("202609080014_InventoryDecisionReports")]
public sealed class InventoryDecisionReports : Migration
{
    protected override void Up(MigrationBuilder migrationBuilder) => migrationBuilder.Sql("""
        ALTER TABLE commercial.inventory_shortlist_candidates ADD COLUMN agent_interpreted boolean;
        ALTER TABLE commercial.shortlist_selections ALTER COLUMN reason TYPE varchar(2000);

        -- A single read projection over immutable selection records. The function exposes
        -- private fields only to the assigned planner, never through the supplier branch.
        CREATE FUNCTION commercial.read_inventory_decisions(p_brief uuid, p_product uuid)
        RETURNS TABLE (
            ...
            "AgentInterpreted" boolean, "DecidedAtUtc" timestamptz,
        ...
        BEGIN
            ...
            WITH campaign_scope AS (
                ...
                SELECT pair.*, item.inventory_tenant_id, item.inventory_product_id,
                    item.product_version_id, item.product_name, item.agent_interpreted,
                ...
                FROM pairs pair CROSS JOIN LATERAL (
                    SELECT COALESCE(c.inventory_tenant_id, old.inventory_tenant_id) AS inventory_tenant_id,
                        ...
                        c.agent_interpreted, c.selection_id, COALESCE(c.is_selected, false) AS is_selected,
            ...
            SELECT COALESCE(d.selection_id, d.decision_id), d.previous_event,
                ...
                d.agent_interpreted, d.selected_at_utc,
            ...
            FROM decisions d
            ORDER BY d.selected_at_utc DESC, d.brief_sequence DESC, d.version_no DESC,
                d.id DESC, d.inventory_product_id LIMIT 101;
        END $function$;
        REVOKE ALL ON FUNCTION commercial.read_inventory_decisions(uuid, uuid) FROM PUBLIC;
        GRANT EXECUTE ON FUNCTION commercial.read_inventory_decisions(uuid, uuid) TO advertified_app;
        """);

    protected override void Down(MigrationBuilder migrationBuilder) =>
        throw new NotSupportedException("Inventory decision evidence is forward-only.");
}
