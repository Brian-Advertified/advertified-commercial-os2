...
{
    private static void AddWorkerFunctions(MigrationBuilder migrationBuilder)
    {
        migrationBuilder.Sql(
            ...
            CREATE FUNCTION commercial.heartbeat_inventory_extraction_attempt(
                p_claim_token uuid, p_lease_seconds integer)
            RETURNS boolean
            LANGUAGE sql
            SECURITY DEFINER
            SET search_path = pg_catalog, commercial
            AS $heartbeat_extraction_attempt$
                UPDATE commercial.inventory_extraction_attempts attempt
