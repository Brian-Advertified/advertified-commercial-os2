        ...
        CancellationToken cancellationToken)
    {
        var snapshot = await dbContext.Database.SqlQuery<MarketplaceProductSnapshotRow>($"""
            SELECT product.id AS "ProductId", version.id AS "ProductVersionId",
                ...
                version.geography AS "Geography", version.outlet_id AS "OutletId",
                version.outlet_name AS "OutletName", version.outlet_identity_basis AS "OutletBasis",
                version.outlet_source_locator AS "OutletSourceLocator",
