...
def enrich(seed: dict) -> dict:
    ...
    for index, record in enumerate(expanded_records, 1):
        ...
        item = {**record, "rowNumber": index, "candidateId": stable_id("candidate", record["key"]),
                ...
                "versionId": stable_id("version", f"{record['key']}:{seed['seedVersion']}"),
                "availabilityId": stable_id("availability", f"{record['key']}:{seed['seedVersion']}"),
        ...
        for rate_index, rate in enumerate(rates, 1):
            item["rates"].append({**rate,
                "id": stable_id("rate", f"{record['key']}:{seed['seedVersion']}:{rate_index}")})
...
def main() -> int:
    ...
    args.output.write_text(
        generate(enrich(seed), governed_codes()), encoding="utf-8", newline="\n")
    print(args.output)
    return 0
