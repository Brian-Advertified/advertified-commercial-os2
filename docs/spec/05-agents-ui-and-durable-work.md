...
| /briefs/new                   | Upload or paste the original client brief before interpretation        | Understand this brief             | File/text source, parse failure, missing source, system records immutable `OOH_ONLY` or `FULL_CAMPAIGN` mode |
...
| /stp/:id                      | Review segmentation, chosen targets and positioning for any campaign mode | Approve STP direction          | Hypothesis labels, exclusions, low confidence, unsupported positioning, OOH geography/movement gaps     |
...
| /automation/inbound-ooh       | Monitor the configured OOH mailbox and its zero-touch proposal runs    | Open exception only               | Duplicate email, non-OOH request, missing brief fact, stale supply, failed render/delivery, safe retry  |
