...
| First production vertical | Build the full-campaign opportunity-to-proposal journey first. OOH-only is not a second planning engine: it uses the identical Brief → STP → mix → shortlist → plan → proposal primitives with an immutable OOH/DOOH-only channel constraint and may be initiated by the configured inbound mailbox. | Product + Commercial       |
