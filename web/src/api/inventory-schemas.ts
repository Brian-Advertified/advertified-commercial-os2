...
export const inventoryCommercialTermsSchema = z.object({
  ...
  inclusions: z.array(requiredText).nullish().transform(value => value ?? []),
  exclusions: z.array(requiredText).nullish().transform(value => value ?? []),
  conditions: z.array(requiredText).nullish().transform(value => value ?? []),
...
export const inventorySpatialSchema = z.object({
  ...
  pointsOfInterest: z.array(pointOfInterestSchema).nullish().transform(value => value ?? []),
