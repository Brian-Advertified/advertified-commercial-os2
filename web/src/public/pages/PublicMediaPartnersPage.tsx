...
const initialPartnerCount = 12;

export function PublicMediaPartnersPage() {
  const [expanded, setExpanded] = useState(false);
  const partners = expanded ? mediaInventoryPartners : mediaInventoryPartners.slice(0, initialPartnerCount);
  return (
    <>
      ...
      <section className="section"><div className="shell"><h2 className="partner-page-heading">MEDIA PARTNERS</h2><p className="lead">These brands are represented by current source records in Advertified's inventory catalogue. Placement, price and availability are verified for each campaign before recommendation or release.</p><div className="partner-cards">{partners.map((partner) => <article key={partner.name}><img className="partner-card-logo" src={partner.assetPath} alt={`${partner.name} logo`} width={partner.width} height={partner.height} /><small>Catalogue source</small><p>Inventory records help planners understand formats, locations and potential campaign roles without implying current availability.</p></article>)}</div>{mediaInventoryPartners.length > initialPartnerCount && <div className="partner-directory-actions"><button className="btn secondary" type="button" onClick={() => setExpanded(value => !value)}>{expanded ? 'Show fewer partners' : `Show all ${mediaInventoryPartners.length} partners`}</button><small>Use the catalogue for current product, price and availability evidence.</small></div>}</div></section>
